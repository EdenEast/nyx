...
in {
  ...
  config = mkIf cfg.enable {
    ...
    nyx.modules.shell.bash.initExtra = mkIf config.nyx.modules.shell.bash.enable ''
    ...
    nyx.modules.shell.bash.profileExtra = mkIf config.nyx.modules.shell.bash.enable ''
    ...
    nyx.modules.shell.zsh.initExtra = mkIf config.nyx.modules.shell.zsh.enable ''
    ...
    nyx.modules.shell.zsh.profileExtra = mkIf config.nyx.modules.shell.zsh.enable ''
      ${optionalString (cfg.useDefaultFd) ''
        export FZF_DEFAULT_COMMAND="fd --type f --hidden --follow --exclude .git"
        export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND"
        export FZF_ALT_C_COMMAND="fd --type d --hidden --follow --exclude .git"
