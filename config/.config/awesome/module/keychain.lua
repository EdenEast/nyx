...
local function default_style()
    ...
    return util.table.merge(style, util.table.check(beautiful, "float.keychain") or {})
end
...
function keychain:init(style)
    ...
    local style = util.table.merge(default_style(), style or {})
    self.style = style
    ...
    self.keygrabber = function(mod, key, event)
        ...
        -- elseif awful.util.table.hasitem(self.service.help,     key) then redtip:show()
            ...
            for _, item in ipairs(self.active[3]) do
                if util.key.match_grabber(item, mod, key) then self:activate(item); return end
            end
        end
    end
end
...
function keychain:activate(item, keytip)
    ...
    if type(item[3]) == "function" then
        ...
        if not self.active then
            util.placement.centered(self.wibox, nil, mouse.screen.workarea)
